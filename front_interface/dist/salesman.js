...
Ring.prototype.tourLength = function () {
  ...
  for (var i = 0; i < this.length; i++) {
    dist += Math.sqrt((current.x - previous.x) * (current.x - previous.x) + (current.y - previous.y) * (current.y - previous.y));
    current = previous;
...
function TravelingSalesman() {
  ...
  this.cycle = 0;
  /* Number of complete survey done */

  this.maxCycles = 1000;
  /* Number of complete suerveys */
...
TravelingSalesman.prototype.surveyRun = function () {
  ...
  if (this.cycle++ % this.update == 0) {
    var length = this.neurons.tourLength(); //this.print();
